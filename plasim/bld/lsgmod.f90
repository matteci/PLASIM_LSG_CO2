...
!
!**** start code Francesco Ragone - 27/10/2023
!      
!      real (kind=8) :: temp(ien,jen,ken)
!
!**** end code Francesco Ragone - 27/10/2023
!
      equivalence(t,ta)
      equivalence(s,sa)
...
!      real (kind=8) :: temp(ien,jen,ken) !LINE BY JEROME SAUER 16 FEBRUARY 2024
!
...
!
!      temp=t+p*(aa1+aa2*t+p*(aa7+aa8*t+p*aa11))
!      temp=t+p*(aa1+(aa2+(aa3+aa4*temp)*temp)*temp)+p*(s-35.)*(aa5+aa6*temp)       &
!     &     +p*p*(aa7+(aa8+aa9*temp)*temp)-aa10*(s-35.)*p*p+p*p*p*(aa11+aa12*temp)
...
!      end do
!
!**** end code Francesco Ragone - 23/10/2023
!
